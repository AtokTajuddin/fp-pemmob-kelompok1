...
@Singleton
class BankLocationRepository @Inject constructor(
    private val placesApiService: PlacesApiService
) {
    private val apiKey = "AIzaSyAKSi7o_HPc5-RHAiK8Hg1a--ZdUgNKqdg"
    ...
    ): Result<List<BankLocation>> = withContext(Dispatchers.IO) {
        try {
            val keyword = when (bankType) {
            ...
            val response = placesApiService.getNearbyBanks(
                location = location,
                radius = radius,
                keyword = keyword,
                apiKey = apiKey
            ...
            if (response.isSuccessful && response.body()?.status == "OK") {
                val banks = response.body()?.results?.mapNotNull { result ->
                    val detectedBankType = detectBankType(result.name)
                    if (bankType == null || bankType == BankType.ALL || detectedBankType == bankType) {
                        BankLocation(
                            id = result.place_id,
                            name = result.name,
                ...
                Log.e("BankLocationRepo", "API Error: $status - ${response.body()?.error_message}")
    ...
    suspend fun searchBanksByQuery(
        ...
        try {
            ...
            val response = placesApiService.searchBanks(
                query = "$query bank Indonesia",
                location = location,
                radius = if (location != null) 100000 else null, // 100km if location provided
                apiKey = apiKey
            )

            if (response.isSuccessful && response.body()?.status == "OK") {
                val banks = response.body()?.results?.map { result ->
                    BankLocation(
                        id = result.place_id,
                        name = result.name,
    ...
    suspend fun searchPlacesFlexible(rawQuery: String): Result<List<BankLocation>> = withContext(Dispatchers.IO) {
        try {
            val response = placesApiService.searchBanks(
                query = rawQuery.trim(),
                location = null,
            ...
            if (response.isSuccessful && response.body()?.status == "OK") {
                ...
                val status = response.body()?.status ?: "UNKNOWN"
                Log.w("BankLocationRepo", "Flexible search API error: $status, returning empty list")
    ...
    private fun detectBankType(name: String): BankType {
        return when {
            ...
            name.contains("Seabank", ignoreCase = true) || name.contains("Sea Bank", ignoreCase = true) -> BankType.SEABANK
            else -> BankType.ALL
        }
    }
    ...
    private fun getMockBankData(userLocation: LatLng, bankType: BankType?): List<BankLocation> {
        val mockBanks = listOf(
            BankLocation(
                ...
                position = LatLng(userLocation.latitude + 0.01, userLocation.longitude + 0.01),
            ...
            BankLocation(
                ...
                position = LatLng(userLocation.latitude + 0.015, userLocation.longitude - 0.01),
            ...
            BankLocation(
                ...
                position = LatLng(userLocation.latitude - 0.012, userLocation.longitude - 0.015),
            ...
            BankLocation(
                ...
                position = LatLng(userLocation.latitude + 0.008, userLocation.longitude + 0.012),
            ...
            BankLocation(
                ...
                position = LatLng(userLocation.latitude - 0.008, userLocation.longitude + 0.008),
